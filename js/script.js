...
// VARIABLES
const storyInfo = document.querySelector(
  '.story__info'
);
const storyInfoNext = document.querySelector(
  '.story__more-info--next'
);
const storyFade = document.querySelector(
  '.story__fade'
);
...
const backInfo = document.querySelector(
...
const animationImages = document.querySelectorAll(
  '.world__image-animation img'
);
const worldImg1 = document.querySelector(
  '.world__img--1'
);
const worldImg2 = document.querySelector(
  '.world__img--2'
);
const closeImg1 = document.querySelector(
  '.close__img--1'
);
const closeImg2 = document.querySelector(
  '.close__img--2'
);
...
backInfo.addEventListener('click', function () {
  ...
  background2.style.transform =
    'translateX(-100%)';
});

// World Explore Span - image animation

exploreSpan.addEventListener(
  'click',
  function () {
    worldImages();
  }
...
function scrollDropdown() {
  ...
  if (y >= 0) {
    dropdownTitle.innerHTML =
      dropdownLinks[0].innerHTML;
  }
  if (y > 725) {
    dropdownTitle.innerHTML =
      dropdownLinks[1].innerHTML;
  }
  if (y > 1500) {
    dropdownTitle.innerHTML =
      dropdownLinks[2].innerHTML;
  }
  if (y > 2300) {
    dropdownTitle.innerHTML =
      dropdownLinks[3].innerHTML;
    if (y > 2600) {
      dropdownTitle.innerHTML =
        dropdownLinks[4].innerHTML;
    } else {
      return;
    }
  }
...
function screenFunction(screen) {
  if (screen.matches) {
    ...
    document.querySelector(
      '.hero__video--title'
    ).innerHTML = 'Death & Beyond'; // If screen is 750px or smaller change 'and' to &
    header.style.display = 'none';
    hamburgerIcon.style.display = 'block';
  } else {
    document.querySelector(
      '.hero__video--title'
    ).innerHTML = 'Death and Beyond';
    header.style.display = 'flex';
    closeIcon.style.display = 'none';
